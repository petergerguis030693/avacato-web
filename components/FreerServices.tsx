...
function LinkRow({ service }: { service: Service }) {
  return (
    <li className="border-b border-navy/10 last:border-b-0">
      ...
      >
        <span className="font-semibold text-ink group-hover:underline group-hover:decoration-gold/60 group-hover:underline-offset-4">
          {service.title}
        </span>
...
export function HomeServicesFreer() {
  ...
  const rest = restList();
  return (
    <section className="bg-cream py-12 sm:py-20 md:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <h2 className="max-w-xl text-2xl font-bold text-navy sm:text-3xl">
          خدماتنا
        </h2>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-20 md:py-24">
      ...
      <div className="mt-14 sm:mt-20">
        <p className="mb-3 text-sm font-semibold tracking-wide text-navy">
          كل الخدمات
        </p>
        <ul className="w-full">
          {services.map((s) => (
